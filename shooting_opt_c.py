# shooting_qlearning_option_c_fixed.py
"""
...
# ---------- Utilities ----------
def plot_training(rewards, epsilons, success_rates, outdir="results_option_c"):
    ...
    plt.xlabel("Episode")
    plt.tight_layout()
    fname = os.path.join(outdir, "training_plots_option_c.png")
    plt.savefig(fname)
    print(f"Saved training plots to {fname}")
    plt.close()

def save_qtable(Q, filename="q_table_option_c.pkl"):
    with open(filename, "wb") as f:
        pickle.dump(Q, f)
    print(f"Saved Q-table to {filename}")

def load_qtable(filename="q_table_option_c.pkl"):
...
def main():
    ...
    parser.add_argument("--save", type=str, default="q_table_option_c.pkl", help="File to save Q-table")
    parser.add_argument("--plot-dir", type=str, default="results_option_c", help="Directory to save plots")
    ...
    print("Training Q-learning agent on ShootingEnvB (Option C)")
